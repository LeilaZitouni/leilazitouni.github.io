...
function loadPage(pageName) {
    if (pageCache[pageName]) {
        ...
        $("#content").attr("w3-include-html", "./personal/page/" + pageName + ".html")
    }
...
function gotoPage(futurePage) {
    if (futurePage == pages.length) {
        futurePage = 0
    }
    if (futurePage < 0) {
        futurePage = pages.length - 1
    }
...
function init() {
    ...
    $("body").on("click", "[lazy-iframe]", function() {
        var iframe = $(
            '<iframe  width="640" height="360" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>'
        )
