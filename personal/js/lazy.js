...
function lazyLoadByAttr(el) {
    for (var i = 0; i < lazyProcessors.length; ++i) {
        ...
        if (
            el.getAttribute(processor[0]) &&
            isVisible(el, window.innerHeight * 0.5, window.innerHeight * 0.5)
        ) {
...
function isVisible(el, extraOffsetTop, extraOffsetBottom) {
    ...
    return (
        (topEdge < bottomScreenEdge && topEdge > topScreenEdge) || // top edge in screen
        (bottomEdge < bottomScreenEdge && bottomEdge > topScreenEdge) || // bottom edge in screen
        (topEdge < topScreenEdge && bottomEdge > bottomScreenEdge)
    ) // belly in screen
}
